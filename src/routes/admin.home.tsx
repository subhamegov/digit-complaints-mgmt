...
function HomePage() {
  ...
  const stats = [
    { label: "Total Complaints", value: s.total.toLocaleString(), icon: Inbox, tone: "text-foreground" },
    { label: "Open Complaints", value: s.open.toLocaleString(), icon: AlertTriangle, tone: "text-amber-600" },
    { label: "Resolved Complaints", value: s.resolved.toLocaleString(), icon: CheckCircle2, tone: "text-emerald-600" },
        ...
        <section className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
          {stats.map((stat) => {
            ...
            return (
              <div
                key={stat.label}
                className="rounded-sm border border-border bg-surface p-3.5"
              >
                ...
                <div className={`mt-1.5 text-[20px] font-semibold ${stat.tone}`}>
                  {stat.value}
                </div>
              </div>
            );
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-sm border border-border bg-surface p-4">
            ...
            <ul className="space-y-2">
              <AlertRow
                tone="amber"
                title={`${s.breached} complaints breached SLA`}
                body="Review escalations under Monitoring & Analytics."
              />
              <AlertRow
                tone="emerald"
                title="Integration health: all systems operational"
                body="GIS, Identity Provider, Notification Gateway responding normally."
              />
              <AlertRow
                tone="slate"
                title={`Reopen rate at ${s.reopenRate}%`}
                body="Within target threshold of 15%."
              />
              <AlertRow
                tone="slate"
                title={`Citizen satisfaction: ${s.satisfaction} / 5`}
                body="Rolling 30-day average across resolved complaints."
              />
            </ul>
          </div>
        </section>
